-- V75__f1_seed_tsunoda.sql — Yuki Tsunoda joins the picker roster.
--
...
-- session's qualifying lock). Seeding him now makes him pickable ahead of the
-- one-off Racing Bulls stint announced in V73's newsletter draft; the "raced
-- for" team on his actual result is still set per-race (race_results.constructor_id,
-- V74), independently of this row's constructor_id, so this is safe even if
-- his stint turns out longer or shorter than expected.
