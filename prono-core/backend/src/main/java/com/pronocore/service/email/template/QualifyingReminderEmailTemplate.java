...
/**
 * F1-only counterpart of {@link RaceReminderEmailTemplate}: fires ahead of qualifying rather
 * than the race start, since the pole pick locks at {@code qualifyingDate} while every other
 * pick (podium, fastest lap, last place) still locks at {@code raceDate}.
 */
public final class QualifyingReminderEmailTemplate {

    private static final DateTimeFormatter TIME_FORMAT = DateTimeFormatter.ofPattern("HH'h'mm", Locale.FRANCE);

    private QualifyingReminderEmailTemplate() {
    }

    public static String subject(List<Race> races) {
        return races.size() == 1
            ? "🏎️ Rappel qualifs : " + races.get(0).getName() + " dans 4h !"
            : "🏎️ Rappel qualifs : " + races.size() + " Grands Prix dans 4h !";
    }
    ...
    public static String build(EmailTheme theme, User user, List<Race> races, String frontendUrl) {
            ...
            return """
                <div style="background:#f8f9fa;border-radius:8px;padding:16px;margin:12px 0;display:flex;align-items:center;justify-content:space-between">
                  <div>
                    ...
                    <div style="color:#6b7280;font-size:13px;margin-top:2px">Qualifs %s • %s</div>
                  </div>
                  <a href="%s"
                ...
                """.formatted(r.getName(), r.getQualifyingDate().format(TIME_FORMAT), r.getCircuit(), appUrl);
        }).collect(Collectors.joining());

        String intro = races.size() == 1
            ? "Les qualifications de la course suivante commencent dans <strong>4 heures</strong> et tu n'as pas encore saisi ton pronostic !"
            : "Les qualifications des <strong>" + races.size() + " Grands Prix</strong> suivants commencent dans <strong>4 heures</strong> et tu n'as encore saisi aucun pronostic !";

        String bodyBase = """
            <h2 style="color:#1a1a1a;margin-top:0">Rappel qualifs ⏰</h2>
            <p style="color:#444;line-height:1.6">Bonjour <strong>%s</strong>,</p>
            <p style="color:#444;line-height:1.6">%s</p>
            <p style="color:#444;line-height:1.6">La pole se verrouille aux qualifs : passé ce délai, tu ne pourras plus la pronostiquer (le reste du prono reste jouable jusqu'au départ).</p>
            %s
        ...
        return EmailLayout.wrap(theme, "🏎️", "Formule 1", bodyBase);
    }
}
