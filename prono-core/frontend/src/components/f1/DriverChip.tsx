...
interface DriverChipProps {
  driver: Driver;
  /** Dimmed when the driver is already placed on a slot. */
  used?: boolean;
...
const DriverChip: React.FC<DriverChipProps> = ({ driver, used, selected, size = 44, onClick }) => (
  <button
    type="button"
    onClick={onClick}
    disabled={used && !onClick}
    className={`flex flex-col items-center gap-0.5 rounded-lg px-1.5 py-1.5 border transition-all select-none touch-none
      ${selected
      ...
      ${used ? 'opacity-30' : 'hover:border-gray-400 dark:hover:border-gray-500 cursor-grab active:cursor-grabbing'}`}
    aria-label={`${driver.name} (${driver.constructorName})`}
    title={`${driver.name} — ${driver.constructorName}`}
  >
